...
function hillClimb(){
 ...
 let vehicle=0,level=0,canvas,ctx,W=0,H=0,dpr=1,seed=Math.random()*10000,world=0,dist=0,coins=0,fuel=100,gas=0,brake=0,dead=false,last=0,raf,score=0,paused=false,checkpoint=0,wheelSpin=0,splash=0,landFx=0,lastAir=0,hitFx=0;
 ...
 function draw(){
  ...
  const slope=Math.atan2(gy(x+22)-gy(x-22),44),cx=W*.34,cy=ground-25;if(car.air>0){lastAir=car.air;car.air-=1}else{if(lastAir>0){landFx=10;car.bounce=-5}car.angle+=(slope-car.angle)*.16;car.airAngle=car.angle}
 ...
 function loop(now){const dt=Math.min((now-last)/16.67,2);last=now;if(dead)return;if(paused){draw();if(splash>0&&level===1){ctx.save();ctx.strokeStyle="#dff";ctx.lineWidth=2;for(let i=0;i<8;i++){let px=W*.34-28+i*8,py=ground+8-(splash%4)*3;ctx.beginPath();ctx.moveTo(px,py);ctx.lineTo(px+(i%2?5:-5),py-12-(i%3)*4);ctx.stroke()}ctx.restore()}raf=requestAnimationFrame(loop);return;}const x=world+W*.34,slope=(gy(x+18)-gy(x-18))/36,v=vehicles[vehicle];car.speed+=gas*v.acc*dt-brake*.17*dt-slope*v.grip*dt;if(car.air>0){car.airAngle+=(gas-brake)*.035*dt;car.airAngle*=Math.pow(.995,dt)}car.speed*=Math.pow(.993,dt);car.speed=Math.max(-2.2,Math.min(v.max,car.speed));world+=Math.max(0,car.speed)*dt*2.55;dist=Math.max(dist,world/7);checkpoint=Math.max(checkpoint,Math.floor(dist/250)*250);wheelSpin+=car.speed*dt*.18;splash=Math.max(0,splash-dt);fuel-=gas*.043*dt;score=Math.floor(dist)+coins*25;
  if(level===1&&x>830&&x<1140){splash=10;if(Math.abs(car.angle)>1.2)end("🌊 Ins Wasser gerutscht!");}const obstacleIndex=Math.floor((x-760)/280);if(obstacleIndex>=0&&obstacleIndex<levels[level].goal/280&&Math.abs(x-(760+obstacleIndex*280+(obstacleIndex%3)*65))<38&&car.air<=0&&Math.abs(slope)<1.2){hitFx=8;car.speed*=.62;score=Math.max(0,score-10);}if(level===2&&x>1260&&x<1450&&Math.abs(car.angle)>1.45)end("🪨 Im Tunnel überschlagen!");
