...
function hillClimb(){
 ...
 function loop(now){const dt=Math.min((now-last)/16.67,2);last=now;if(dead)return;if(paused){draw();raf=requestAnimationFrame(loop);return;}const x=world+W*.34,slope=(gy(x+18)-gy(x-18))/36,v=vehicles[vehicle];car.speed+=gas*v.acc*dt-brake*.17*dt-slope*v.grip*dt;car.speed*=Math.pow(.993,dt);car.speed=Math.max(-2.2,Math.min(v.max,car.speed));world+=Math.max(0,car.speed)*dt*2.55;dist=Math.max(dist,world/7);checkpoint=Math.max(checkpoint,Math.floor(dist/250)*250);wheelSpin+=car.speed*dt*.18;fuel-=gas*.043*dt;score=Math.floor(dist)+coins*25;
