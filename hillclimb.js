...
function hillClimb(){
 ...
 let vehicle=0,level=0,canvas,ctx,W=0,H=0,dpr=1,seed=Math.random()*10000,world=0,dist=0,coins=0,fuel=100,gas=0,brake=0,dead=false,last=0,raf,score=0;
 const car={angle:0,speed:0,bounce:0,air:0,airAngle:0};
 function choose(){
  box.innerHTML='<div class="hillSelect"><h2>🚙 Hill Climb Racing</h2><p>Wähle Fahrzeug und Strecke</p><div class="hillCars">'+vehicles.map((v,i)=>'<button data-v="'+i+'">'+v.name+'<small>'+(i===0?"Ausgeglichen":i===1?"Stabil":"Schnell")+'</small></button>').join("")+'</div><div class="hillLevels">'+levels.map((l,i)=>'<button data-l="'+i+'">'+l.name+'<small>'+l.goal+' m</small></button>').join("")+'</div></div>';
 ...
 function draw(){
  ...
  ctx.fillStyle=v.color;ctx.beginPath();ctx.roundRect(-30,-13,60,25,5);ctx.fill();ctx.fillStyle="#c9efff";ctx.beginPath();ctx.moveTo(-14,-13);ctx.lineTo(-4,-24);ctx.lineTo(12,-24);ctx.lineTo(19,-13);ctx.closePath();ctx.fill();circle(-19,13,10,"#222");circle(19,13,10,"#222");circle(-19,13,4,"#aaa");circle(19,13,4,"#aaa");ctx.fillStyle="#fff";ctx.font="bold 8px sans-serif";ctx.textAlign="center";ctx.fillText("GETA",0,2);ctx.restore();
  document.getElementById("hcDist").textContent=Math.floor(dist)+" m";document.getElementById("hcCoins").textContent="🪙 "+coins;document.getElementById("hcFuel").textContent="⛽ "+Math.max(0,Math.floor(fuel))+"%";document.getElementById("hcScore").textContent="⭐ "+Math.floor(score);document.getElementById("hcLevel").textContent=L.name;
 }
 ...
 function start(){
  box.innerHTML='<canvas></canvas><div class="hillHud"><span id="hcLevel">'+levels[level].name+'</span><span id="hcDist">0 m</span><span id="hcCoins">🪙 0</span><span id="hcFuel">⛽ 100%</span><span id="hcScore">⭐ 0</span></div><div class="hillHint">◀ Bremsen &nbsp; ▶ Gas</div><div class="hillControls"><button id="hcBrake">◀</button><button id="hcGas">▶</button></div>';
  canvas=box.querySelector("canvas");ctx=canvas.getContext("2d");resize();addEventListener("resize",resize);const btn=(el,on)=>{el.onpointerdown=e=>{e.preventDefault();on(1)};el.onpointerup=el.onpointercancel=()=>on(0)};btn(box.querySelector("#hcGas"),v=>gas=v);btn(box.querySelector("#hcBrake"),v=>brake=v);
  world=0;dist=0;coins=0;fuel=100;gas=0;brake=0;dead=false;score=0;car.angle=0;car.speed=0;car.air=0;car.airAngle=0;last=performance.now();loop(last)
 }
 function loop(now){const dt=Math.min((now-last)/16.67,2);last=now;if(dead)return;const x=world+W*.34,slope=(gy(x+18)-gy(x-18))/36,v=vehicles[vehicle];car.speed+=gas*v.acc*dt-brake*.17*dt-slope*v.grip*dt;car.speed*=Math.pow(.993,dt);car.speed=Math.max(-2.2,Math.min(v.max,car.speed));world+=Math.max(0,car.speed)*dt*2.55;dist=Math.max(dist,world/7);fuel-=gas*.043*dt;score=Math.floor(dist)+coins*25;
  if(level===1&&x>830&&x<1140&&Math.abs(car.angle)>1.2)end("🌊 Ins Wasser gerutscht!");if(level===2&&x>1260&&x<1450&&Math.abs(car.angle)>1.45)end("🪨 Im Tunnel überschlagen!");
