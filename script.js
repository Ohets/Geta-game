...
function flightSimReal(){
 ...
 const viewBar=document.createElement("div");viewBar.style.cssText="position:absolute;left:50%;top:4px;transform:translateX(-50%);z-index:25;display:flex;gap:2px;flex-wrap:wrap;justify-content:center;width:46%;pointer-events:auto";
...
const cockpitHud=document.createElement("div");cockpitHud.style.cssText="position:absolute;inset:0;pointer-events:none;z-index:18;display:none;color:#d9f5ff;font-family:monospace";
 ...
 const hud=document.createElement("div");hud.className="flightHud";b.appendChild(hud);
 const select=document.createElement("select");select.innerHTML='<option value="A320">✈️ A320</option><option value="A350">✈️ A350</option><option value="B737">✈️ B737</option><option value="A380">✈️ A380</option><option value="B787">✈️ B787</option><option value="EVTOL">🚁 EVTOL</option><option value="Drone">🚁 Drone</option>';select.style.cssText="position:absolute;left:8px;top:66px;z-index:45;padding:4px;font-size:10px;max-width:92px";b.appendChild(select);
 const weather=document.createElement("select");weather.innerHTML='<option value="clear">☀️ Klar</option><option value="rain">🌧️ Regen</option><option value="night">🌙 Nacht</option>';weather.style.cssText="position:absolute;left:8px;top:101px;z-index:45;padding:4px;font-size:10px;max-width:92px";b.appendChild(weather);
 ...
 const callout=document.createElement("div");callout.style.cssText="position:absolute;left:50%;bottom:34%;transform:translateX(-50%);z-index:27;font:bold 15px monospace;color:#ffd84a;text-shadow:0 1px 3px #000;pointer-events:none";b.appendChild(callout);
const cockpitStatus=document.createElement("div");cockpitStatus.style.cssText="position:absolute;right:8px;top:140px;z-index:45;background:rgba(5,10,16,.78);border:1px solid rgba(180,220,230,.45);border-radius:6px;padding:6px 8px;font-size:10px;line-height:1.45;pointer-events:none";cockpitStatus.innerHTML='GEAR <span id="cpGearState">DOWN</span> &nbsp; FLAPS <span id="cpFlapsState">EXT</span><br>SPD BRK <span id="cpBrakeState">ARM</span> &nbsp; AP <span id="cpApState">OFF</span>';b.appendChild(cockpitStatus);
 const extras=document.createElement("div");extras.style.cssText="position:absolute;right:8px;bottom:8px;width:46%;z-index:40;display:grid;grid-template-columns:repeat(4,1fr);gap:3px";extras.innerHTML='<button data-x="gear">🛬 Fahrwerk</button><button data-x="flaps">🪽 Klappen</button><button data-x="brake">🛑 Bremse</button><button data-x="ap">🧭 AP</button><button data-x="trimUp">▲ Trimm</button><button data-x="trimDown">▼ Trimm</button><button data-x="pause">⏸️</button><button data-x="reset">↻</button>';b.appendChild(extras);
...
function loadRealGLB(url,group,done){
 ...
 const finish=model=>{
  ...
  // The GLB aircraft already contains its own orientation quaternion.\n  // Its local aircraft axis is transformed by the GLB node, so do not add\n  // an extra Euler rotation here; doing so flips the nose away from flight direction.\n  if(/A320|A350|B737|A380|B787|EVTOL|drone/i.test(url)) instance.rotation.set(0,-Math.PI/2,0);
  group.add(instance);
