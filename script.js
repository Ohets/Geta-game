...
function flightSimReal(){
 ...
 cockpitHud.innerHTML='<div style="position:absolute;left:10%;bottom:10%;width:80%;height:22%;border:1px solid rgba(180,220,230,.45);background:rgba(10,18,24,.55);border-radius:8px"></div><div style="position:absolute;left:12%;bottom:17%;font-size:10px">PFD &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ND</div><div style="position:absolute;left:15%;bottom:22%;font-size:15px">▣ ALT <span id="cpAlt">120</span>m</div><div style="position:absolute;left:55%;bottom:22%;font-size:15px">HDG <span id="cpHdg">000</span>°</div><div style="position:absolute;left:42%;bottom:7%;font-size:10px">A320 COCKPIT</div>';b.appendChild(cockpitHud);
 ...
 updateGLBVisibility(world,plane);hud.innerHTML="ALT "+Math.round(alt)+" m<br>SPEED "+Math.round(speed)+" km/h<br>VS "+(verticalSpeed>=0?"+":"")+Math.round(verticalSpeed*10)+" m/s<br>HDG "+String(Math.round(heading)).padStart(3,"0")+"°<br>THR "+power+"%<br>FUEL "+Math.round(fuel)+"%<br>"+(gear?"GEAR DOWN":"GEAR UP")+"<br>"+(flaps?"FLAPS":"CLEAN")+"<br>"+(paused?"PAUSED":"MISSION "+(mission+1)+"/4");let target,look;if(viewMode==="cockpit"){if(!cockpitGroup){cockpitGroup=makeA320Cockpit();plane.add(cockpitGroup)}cockpitGroup.visible=selected==="A320";target=new THREE.Vector3(plane.position.x,plane.position.y+1.45,plane.position.z-1);look=new THREE.Vector3(plane.position.x,plane.position.y+1.45,plane.position.z-35)}else if(viewMode==="wing"){target=new THREE.Vector3(plane.position.x+5,plane.position.y+2,plane.position.z+4);look=new THREE.Vector3(plane.position.x,plane.position.y,plane.position.z-25)}else if(viewMode==="cabin"){target=new THREE.Vector3(plane.position.x,plane.position.y+1.3,plane.position.z+2);look=new THREE.Vector3(plane.position.x,plane.position.y+1.4,plane.position.z-25)}else{target=new THREE.Vector3(plane.position.x*.55,plane.position.y+3.1,plane.position.z+cameraDistance);look=new THREE.Vector3(plane.position.x,plane.position.y+.15,plane.position.z-6)}g.camera.position.lerp(target,.10);g.camera.lookAt(look);g.renderer.render(g.scene,g.camera);activeAnimation=requestAnimationFrame(loop)}
