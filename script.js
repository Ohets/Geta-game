...
function flightSimReal(){
 ...
 cockpitHud.innerHTML='<div style="position:absolute;left:7%;top:7%;width:38%;height:54%;border:1px solid rgba(180,220,230,.45);background:rgba(5,12,18,.72);border-radius:8px;overflow:hidden"><div id="cpHorizon" style="position:absolute;inset:10%;border:1px solid rgba(255,255,255,.35);border-radius:50%;overflow:hidden;background:linear-gradient(#4389c7 0 49%,#76563b 50% 100%);transform:rotate(0deg)"><div style="position:absolute;left:15%;right:15%;top:49%;height:2px;background:#fff"></div><div style="position:absolute;left:32%;right:32%;top:39%;height:1px;background:#fff"></div><div style="position:absolute;left:32%;right:32%;top:59%;height:1px;background:#fff"></div><div style="position:absolute;left:42%;top:48%;width:16%;height:2px;background:#ffd44d"></div></div><div style="position:absolute;left:4px;top:4px;font-size:9px">PFD</div><div style="position:absolute;left:5px;bottom:4px;font-size:9px">PITCH <span id="cpPitch">0</span>°</div><div id="cpBank" style="position:absolute;right:5px;top:4px;font-size:9px">BANK 0°</div></div><div style="position:absolute;right:7%;top:7%;width:42%;height:54%;border:1px solid rgba(180,220,230,.45);background:rgba(5,12,18,.72);border-radius:8px"><div style="position:absolute;left:8px;right:8px;top:8px;font-size:9px">A320 PRIMARY FLIGHT DATA</div><div style="position:absolute;left:12px;top:28px;font-size:14px">SPD <span id="cpSpeed">0</span></div><div style="position:absolute;left:12px;top:52px;font-size:14px">ALT <span id="cpAlt">120</span>m</div><div style="position:absolute;left:12px;top:76px;font-size:13px">VS <span id="cpVs">+0</span> m/s</div><div style="position:absolute;right:12px;top:28px;font-size:14px">HDG <span id="cpHdg">000</span>°</div><div style="position:absolute;right:12px;top:52px;font-size:13px">TRIM <span id="cpTrim">0.0</span>°</div><div style="position:absolute;right:12px;top:76px;font-size:13px"><span id="cpAp">MANUAL</span></div></div><div style="position:absolute;left:8%;bottom:8%;width:84%;height:25%;border:1px solid rgba(180,220,230,.45);background:rgba(10,18,24,.62);border-radius:8px;padding:7px;box-sizing:border-box;font-size:11px"><div>FLIGHT DISPLAY</div><div style="margin-top:6px">SPD <span id="cpSpeed2">0</span> &nbsp; ALT <span id="cpAlt2">120</span>m &nbsp; HDG <span id="cpHdg2">000</span>° &nbsp; VS <span id="cpVs2">+0</span></div><div style="margin-top:5px">PITCH <span id="cpPitch2">0</span>° &nbsp; BANK <span id="cpBank2">0</span>° &nbsp; THR <span id="cpThr">60</span>%</div></div><div style="position:absolute;left:40%;bottom:2%;font-size:9px">A320 COCKPIT • FLIGHT DISPLAY</div>';b.appendChild(cockpitHud);
 ...
 function bindCockpitControls(){
  ...
  if(c.throttle0){
   [c.throttle0,c.throttle1].forEach(h=>{
    ...
    h.addEventListener("pointermove",ev=>{if(!h.userData.dragging)return;const dy=ev.movementY||ev.clientY-(h.userData.lastY||ev.clientY);h.userData.lastY=ev.clientY;throttle.value=Math.max(0,Math.min(100,+throttle.value-dy*.45));});
  ...
  if(c.sidestick){
   ...
   c.sidestick.addEventListener("pointermove",ev=>{
    ...
    const dx=ev.movementX||ev.clientX-(c.sidestick.userData.lastX||ev.clientX),dy=ev.movementY||ev.clientY-(c.sidestick.userData.lastY||ev.clientY);c.sidestick.userData.lastX=ev.clientX;c.sidestick.userData.lastY=ev.clientY;
