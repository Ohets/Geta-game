...
function flightSimReal(){
 ...
 updateGLBVisibility(world,plane);hud.innerHTML="ALT "+Math.round(alt)+" m<br>SPEED "+Math.round(speed)+" km/h<br>HDG "+String(Math.round(heading)).padStart(3,"0")+"°<br>THR "+power+"%<br>"+(gear?"GEAR DOWN":"GEAR UP")+"<br>MISSION "+(mission+1)+"/4";let target,look;if(viewMode==="cockpit"){if(!cockpitGroup){cockpitGroup=makeA320Cockpit();plane.add(cockpitGroup)}cockpitGroup.visible=selected==="A320";target=new THREE.Vector3(plane.position.x,plane.position.y+1.45,plane.position.z-1);look=new THREE.Vector3(plane.position.x,plane.position.y+1.45,plane.position.z-35)}else if(viewMode==="wing"){target=new THREE.Vector3(plane.position.x+5,plane.position.y+2,plane.position.z+4);look=new THREE.Vector3(plane.position.x,plane.position.y,plane.position.z-25)}else if(viewMode==="cabin"){target=new THREE.Vector3(plane.position.x,plane.position.y+1.3,plane.position.z+2);look=new THREE.Vector3(plane.position.x,plane.position.y+1.4,plane.position.z-25)}else{target=new THREE.Vector3(plane.position.x*.55,plane.position.y+3.1,plane.position.z+10);look=new THREE.Vector3(plane.position.x,plane.position.y+.15,plane.position.z-2)}g.camera.position.lerp(target,.10);g.camera.lookAt(look);g.renderer.render(g.scene,g.camera);activeAnimation=requestAnimationFrame(loop)}
