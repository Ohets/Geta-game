...
function flightSimReal(){
 ...
 loadRealGLB("https://raw.githubusercontent.com/Ysurac/FlightAirMap-3dmodels/master/a320/glTF2/A320.glb",plane,ok=>{if(ok){msg.textContent="🟢 ECHTES A320-3D-MODELL GELADEN"}else{msg.textContent="🔴 A320-GLB konnte nicht geladen werden";const err=document.createElement("div");err.style.cssText="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);background:#700;color:#fff;padding:10px;border-radius:8px;font-weight:bold;z-index:20";err.textContent="A320-Modell nicht erreichbar";b.appendChild(err)}});
 function loop(now){if(!alive)return;const dt=Math.min((now-last)/16,2);last=now;const power=+throttle.value,speed=.025+power*.00075;world.position.z+=speed*dt;distance+=speed*dt*.2;const targetAlt=takeoff?1800:Math.max(500,1800+py*650);alt+=(targetAlt-alt)*.015*dt;heading=(heading+px*.12*dt+360)%360;plane.position.x=px;plane.position.y=py;plane.rotation.z=-px*.07;plane.rotation.x=-py*.035;
...
function addRealAirliners(world){
 const models=[
  ["A320","https://raw.githubusercontent.com/Ysurac/FlightAirMap-3dmodels/master/a320/glTF2/A320.glb"],
