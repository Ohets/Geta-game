...
function flightSimReal(){
 ...
 const viewBar=document.createElement("div");viewBar.style.cssText="position:absolute;left:50%;top:7%;transform:translateX(-50%);z-index:50;display:flex;gap:2px;flex-wrap:nowrap;justify-content:center;width:62%;pointer-events:auto";
 viewBar.innerHTML='<button style="font-size:9px;padding:3px 4px" data-view="cockpit">🛫 Cockpit</button><button style="font-size:9px;padding:3px 4px" data-view="external">🌍 Außen</button><button style="font-size:9px;padding:3px 4px" data-view="wing">🪽 Flügel</button><button style="font-size:9px;padding:3px 4px" data-view="cabin">💺 Kabine</button>';b.appendChild(viewBar);
 const flightDirector=document.createElement("div");flightDirector.style.cssText="position:absolute;left:34%;top:34%;width:32%;height:20%;pointer-events:none;z-index:20";flightDirector.innerHTML='<div style="position:absolute;left:50%;top:0;width:3px;height:100%;background:#e7d33f"></div><div style="position:absolute;left:0;top:50%;width:100%;height:3px;background:#e7d33f"></div>';const landingGuide=document.createElement("div");landingGuide.style.cssText="position:absolute;left:18%;top:9%;width:64%;height:82%;pointer-events:none;z-index:21";
landingGuide.innerHTML='<div style="position:absolute;left:50%;top:12%;width:2px;height:76%;background:rgba(255,255,255,.55)"></div><div id="ilsLoc" style="position:absolute;left:50%;top:50%;width:4px;height:48px;background:#e6d34a;transform:translate(-50%,-50%)"></div><div id="ilsGs" style="position:absolute;left:50%;top:50%;width:48px;height:4px;background:#e6d34a;transform:translate(-50%,-50%)"></div>';
const cockpitHud=document.createElement("div");cockpitHud.style.cssText="position:absolute;inset:0;pointer-events:none;z-index:18;display:none;color:#d9f5ff;font-family:monospace";
 ...
 const hud=document.createElement("div");hud.className="flightHud";b.appendChild(hud);
 const select=document.createElement("select");select.innerHTML='<option value="A320">✈️ A320</option><option value="A350">✈️ A350</option><option value="B737">✈️ B737</option><option value="A380">✈️ A380</option><option value="B787">✈️ B787</option><option value="EVTOL">🚁 EVTOL</option><option value="Drone">🚁 Drone</option>';select.style.cssText="position:absolute;left:3%;top:24%;z-index:50;padding:4px;font-size:10px;max-width:92px";b.appendChild(select);
 const weather=document.createElement("select");weather.innerHTML='<option value="clear">☀️ Klar</option><option value="rain">🌧️ Regen</option><option value="night">🌙 Nacht</option>';weather.style.cssText="position:absolute;left:3%;top:31%;z-index:50;padding:4px;font-size:10px;max-width:92px";b.appendChild(weather);
 ...
 const callout=document.createElement("div");callout.style.cssText="position:absolute;left:50%;bottom:24%;transform:translateX(-50%);z-index:27;font:bold 15px monospace;color:#ffd84a;text-shadow:0 1px 3px #000;pointer-events:none";b.appendChild(callout);
const cockpitStatus=document.createElement("div");cockpitStatus.style.cssText="position:absolute;right:3%;top:22%;z-index:45;background:rgba(5,10,16,.78);border:1px solid rgba(180,220,230,.45);border-radius:6px;padding:6px 8px;font-size:10px;line-height:1.45;pointer-events:none";cockpitStatus.innerHTML='GEAR <span id="cpGearState">DOWN</span> &nbsp; FLAPS <span id="cpFlapsState">EXT</span><br>SPD BRK <span id="cpBrakeState">ARM</span> &nbsp; AP <span id="cpApState">OFF</span>';b.appendChild(cockpitStatus);
 const extras=document.createElement("div");extras.style.cssText="position:absolute;right:3%;bottom:3%;width:48%;z-index:50;display:grid;grid-template-columns:repeat(4,1fr);gap:3px";extras.innerHTML='<button data-x="gear">🛬 Gear</button><button data-x="flaps">🪽 Flaps</button><button data-x="brake">🛑 Brake</button><button data-x="ap">🧭 AP</button><button data-x="trimUp">▲ Trim+</button><button data-x="trimDown">▼ Trim−</button><button data-x="pause">⏸</button><button data-x="reset">↻</button>';b.appendChild(extras);
 let selected="A320",px=0,py=.25,heading=0,alt=120,speed=0,verticalSpeed=0,bank=0,pitch=0,yawInput=0,turnHeld=0,trim=0,last=performance.now(),alive=true,gear=true,flaps=false,brake=false,mission=0,fuel=100,paused=false,autopilot=false,apHeading=0,apAltitude=120,cameraDistance=10;
 function loadAircraft(){plane.clear();msg.textContent="🛫 "+selected+" wird geladen …";loadRealGLB(urls[selected],plane,ok=>{if(ok&&plane.children[0])plane.children[0].rotation.y=Math.PI;msg.textContent=ok?"🟢 "+selected+" – Welt geladen":"🔴 Flugzeugmodell konnte nicht geladen werden"})}
 ...
 function steer(dx,dy){px=Math.max(-5.5,Math.min(5.5,px-dx));pitch=Math.max(-12,Math.min(12,pitch+dy*7))}
 function yaw(v){yawInput=Math.max(-1,Math.min(1,yawInput-v))}
 const leftBtn=controls.querySelector('[data-fs="left"]'),rightBtn=controls.querySelector('[data-fs="right"]');
 leftBtn.onpointerdown=()=>{steer(-.32,0);turnHeld=1};rightBtn.onpointerdown=()=>{steer(.32,0);turnHeld=-1};
 leftBtn.onpointerup=leftBtn.onpointercancel=()=>turnHeld=0;rightBtn.onpointerup=rightBtn.onpointercancel=()=>turnHeld=0;
 controls.querySelector('[data-fs="up"]').onpointerdown=()=>steer(0,.3);controls.querySelector('[data-fs="down"]').onpointerdown=()=>steer(0,-.3);
 controls.querySelector('[data-fs="yawL"]').onpointerdown=()=>turnHeld=1;controls.querySelector('[data-fs="yawR"]').onpointerdown=()=>turnHeld=-1;
 controls.querySelector('[data-fs="yawL"]').onpointerup=controls.querySelector('[data-fs="yawL"]').onpointercancel=()=>turnHeld=0;controls.querySelector('[data-fs="yawR"]').onpointerup=controls.querySelector('[data-fs="yawR"]').onpointercancel=()=>turnHeld=0;
 document.onkeydown=e=>{if(e.key==="ArrowLeft"){steer(-.18,0);yawInput=.65}if(e.key==="ArrowRight"){steer(.18,0);yawInput=-.65}if(e.key==="ArrowUp")steer(0,.2);if(e.key==="ArrowDown")steer(0,-.2);if(e.key==="a"||e.key==="A")yawInput=.65;if(e.key==="d"||e.key==="D")yawInput=-.65;if(e.key==="w"||e.key==="W")throttle.value=Math.min(100,+throttle.value+5);if(e.key==="s"||e.key==="S")throttle.value=Math.max(0,+throttle.value-5);if(e.key==="p"||e.key==="P"){paused=!paused;msg.textContent=paused?"⏸️ Pause":"▶️ Flug fortgesetzt"}if(e.key==="t"||e.key==="T"){autopilot=!autopilot;if(autopilot){apHeading=heading;apAltitude=alt;msg.textContent="🧭 Autopilot EIN"}else msg.textContent="🧭 Autopilot AUS"}};
...
function addRealAirliners(world){
 ...
 models.forEach((m,i)=>{
  const g=new THREE.Group();g.position.set(-12+i*12,1.8,-35-i*25);g.rotation.y=0;g.scale.setScalar(1);world.add(g);
